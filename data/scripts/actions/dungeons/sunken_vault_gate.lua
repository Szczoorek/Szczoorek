--[[
	...
	Map setup:
	  - Place the lever with actionid 10002 (registered by this script's own
	    aid call at the bottom - no separate XML registration needed).
...
local sunkenVaultGate = Action()

local GATE_UNIQUE_ID = 10010

function sunkenVaultGate.onUse(player, item, fromPosition, target, toPosition, isHotkey)
...
sunkenVaultGate:aid(10002)
sunkenVaultGate:register()
