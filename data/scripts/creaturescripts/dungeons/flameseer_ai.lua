...
local flameseerAIThink = CreatureEvent('FlameseerAIThink')

function flameseerAIThink.onThink(creature, interval)
...
flameseerAIThink:register()

local flameseerAIDeath = CreatureEvent('FlameseerAIDeath')

function flameseerAIDeath.onDeath(creature, corpse, killer, mostDamageKiller, unjustified, mostDamageUnjustified)
	local monster = creature
	...
	DungeonLib.grantRenownToNearby(position, 90, 10)
